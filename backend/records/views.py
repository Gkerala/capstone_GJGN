...
class WeeklyStatAPIView(APIView):
    ...
    def get(self, request):
        ...
        today = date.today()
        start_week = today - timedelta(days=today.weekday())   # 월요일
        end_week = start_week + timedelta(days=6)              # 일요일

        week_data = []

        # 월요일 → 일요일까지 7일 반복
        for i in range(7):
            target_day = start_week + timedelta(days=i)

            foods = MealFood.objects.filter(
                record__user=user,
                record__meal_time__date=target_day
            )

            total = foods.aggregate(cal=Sum("calories"))["cal"] or 0
        ...
        return Response({
            ...
            "weekly": week_data     # ← 그래프용 데이터!
        })

@api_view(['GET'])
@permission_classes([IsAuthenticated])
def weekly_calories_stat(request):
    user = request.user
    ...
    records = MealRecord.objects.filter(
        user=user,
        meal_time__date__range=[week_ago, today]
    ).order_by('meal_time')
    ...
    for rec in records:
        data.append({
            "date": rec.meal_time.strftime("%Y-%m-%d"),
